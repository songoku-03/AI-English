...
This file provides guidance to AI coding agents (Google Antigravity, and any agent that reads AGENTS.md) when working with code in this repository. It mirrors CLAUDE.md — keep the two files in sync.
...
**Before writing or modifying ANY Swift code in this repository, you MUST load and follow the `swift-best-practices` skill** ([.agent/skills/swift-best-practices/SKILL.md](.agent/skills/swift-best-practices/SKILL.md)). This is not optional guidance — code that violates the skill's rules must not be committed. The glob rule [.agent/rules/swift-best-practices.md](.agent/rules/swift-best-practices.md) enforces this automatically whenever a `.swift` file is touched; the `/init` workflow loads it up front. (Claude Code has the identical skill at `.claude/skills/swift-best-practices/SKILL.md` — keep the two copies in sync.)
